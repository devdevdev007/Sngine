...
// chat box
function chat_box(user_id, conversation_id, name, name_list, multiple, link, picture) {
  /* open the #chat_key */
...
async function init_Twilio(type, token, room_name, call_id) {
  ...
  if (type == "video") {
    ...
    divLocal.appendChild(localVideoTrack.attach());
    $('.twilio-stream-local').html(divLocal).show();
  }
  ...
  room.participants.forEach(participant => {
    participant.videoTracks.forEach(publication => {
      if (publication.track) {
        $('.twilio-stream')[0].appendChild(publication.track.attach());
      }
    });
    participant.on('trackSubscribed', track => {
      $('.twilio-stream')[0].appendChild(track.attach());
    });
  });
...
$(function () {
  ...
  $('body').on('click', '.js_chat-toggle', function (e) {
    ...
    $.get(api['chat/settings'], { 'user_chat_enabled': (status == "on") ? 0 : 1 }, function (response) {
      /* check the response */
      if (!response) return;
  ...
  $('body').on('click', '.js_chat-start', function (e) {
    ...
    if (window.location.pathname.indexOf("messages") != -1 && conversation_id) {
      ...
      if (!chat_enabled || $(window).width() < 970) { // Desktops (≥992px)
        ...
        /* desktop view */
        if (chatbox_enabled) {
          /* chat_box is set so return (return will allow default behave of anchor tag) */
          return;
        }
        e.preventDefault();
        /* load chat-box */
  ...
  $('body').on('click', '.js_leave-conversation', function () {
    confirm(__['Delete Conversation'], __['Are you sure you want to delete this conversation?'], function () {
      $.post(api['chat/reaction'], { 'do': 'leave', 'conversation_id': $('.panel-messages').data('cid') }, function (response) {
        if (response.callback) {
          eval(response.callback);
        }
      }, 'json')
